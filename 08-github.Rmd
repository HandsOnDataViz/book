...
Finally, we'll discuss how to [fix common GitHub and code errors](fix-code.html). All of us make mistakes and accidentally "break our code" from time to time, and it's a great way to learn how things work---and what to do when it doesn't work!
...
Note: GitHub creates two different types of web addresses, where you should replace `YourUserName` and `YourRepoName` with your own:

- The GitHub repo address, where you can edit your code: `https://github.com/YourUserName/YourRepoName`
- The GitHub Pages address, which publishes a live version of your code: `https://YourUsername.github.io/YourRepoName`
...
But what if you're trying to make a second copy of a GitHub repo where the Fork button is grayed-out and there's no green "Use this template" button? The simplest solution is to follow these general sequence of steps:
...
Your new repo will have a web address similar to `https://github.com/YourUserName/practice`.
...
(ref:repo-upload-files) Drag-and-drop the file to the upload screen.
...
11. Go to GitHub Desktop, and if your Current Repo is `leaflet-map-simple`, you will see your code edits summarized on the screen. Click the blue Commit to Master button at the bottom of the page to save your edits to your local copy of your repo. Next, click the blue "Push origin" button to transfer those edits to the parent copy of your repo on your GitHub web account. This two-step process is shown in Figure \@ref(fig:desktop-commit-push).

(ref:desktop-commit-push) **FIX CAPTION**

```{r desktop-commit-push, fig.cap="(ref:desktop-commit-push)"}
 knitr::include_graphics("images/08-github/desktop-commit-push.gif")
```

**START AGAIN HERE**
PUT TWO static images together
Go see if edits appear on your live map, hosted with GitHub Pages, at .....
Summarize the value of GitHub Desktop.... more complex code edits, and transfer files directly to the repo in your local computer.... testing....
Restore leaflet-simple-map to its pre-edited state
...
GitHub is a powerful platform for collaborative projects. For example, we collaboratively composed the text of this book and all of the code templates on GitHub. Jack started each day by "pulling" the most recent version of the manuscript from our shared GitHub account to his local computer with GitHub Desktop, where he worked on sections and "pushed" his commits (aka edits) back to GitHub. At the same time, Ilya "pulled" the latest version and "pushed" his commits back to GitHub as well. Both of us can view the commits that each other made, line-by-line in green and red (showing additions and deletions), by selecting the GitHub repo "Code" tab and clicking one of our commits, as shown in Figure \@ref(fig:compare-commits).
...
While GitHub is not like Google Documents, which displays live editing of text by co-authors, the platform has several other advantages when working together with code. First, since GitHub tracks every code commit we make, it allows us to go back and restore a very specific past version of the code if needed. Second, when GitHub repos are public, anyone can view your code and submit an "issue" to notify the owner about a problem or idea, or send a "pull request" of suggested code edits, which the owner can accept or reject. Third, GitHub allows collaborators to create different "branches" of a repo (the default is called "master"), make their own edits, and then "merge" the branches back together if desired. Occasionally, if two or more coders attempt to push incompatible commits to the same repo, GitHub will warn about a "Merge Conflict." To resolve this conflict and preserve everyone's work, you may need to use the Command Line Interface (CLI) version of GitHub, which means typing commands directly into the Terminal application on Mac or Windows. Many professional coders regularly work on the Command Line with GitHub, but this requires memorizing a list of commands and is beyond the scope of this introductory book.
