...
```{r transform-data, fig.cap="(ref:transform-data)"}
...
Both are powerful, web-based open-source tools that can substitute ArcGIS and QGIS for certain common tasks.
...
If you have just a handful of addresses, it might be faster to use Google Maps to geocode them.
Search for an address, right-click on the map and select *What's here?*.
A popup window with latitude and longitude values will appear, and you can record them
in your spreadsheet. But what if you need to geocode dozens, hundreds, or even thousands of addresses?

In this section, we will look at two ways to geocode lists of addresses: using our custom-built
[Google Sheets Geocoder](https://github.com/HandsOnDataViz/google-sheets-geocoder), which
lets you choose between Google Geocoder (available pretty much worldwide) and the US Census Geocoder suitable for US addresses,
and a stand-alone US Census Geocoder that lets you upload address files and get geocoded results.

#### Geocode addresses with Google Sheets Geocoder {-}

The Google Sheets Geocoder script lives inside a [Google Sheet](https://docs.google.com/spreadsheets/d/1XvtkzuVyQ_7Ud47ypDJ4KOmz_5lOpC9sqeEDBbJ5Pbg/edit#gid=0) that you can copy to your own Google Drive
(you don't need editing access, just go to *File > Make a copy*).
...
go to *Geocoder* in the menu, and choose which geocoding utility to use, like is shown in Figure \@ref(fig:google-sheets-geocoder).

(ref:google-sheets-geocoder) Put addresses in the first column, and use Geocoder to fill in the remaining five.

```{r google-sheets-geocoder, fig.cap="(ref:google-sheets-geocoder)"}
if(knitr::is_html_output()) knitr::include_url("images/11-transform/google-sheets-geocoder-census-google.gif", height="280px") else knitr::include_graphics("images/11-transform/google-sheets-geocoder.png")
```
...
Then look at the failed addresses and see if you can spot problems with them. For tops about
...
and go to *Geocoder > US Census 2010 Geographies*, like is shown in Figure \@ref(fig:google-sheets-geocoder-census).
...
(ref:google-sheets-geocoder-census) Put addresses in the first column, and use Geocoder to fill in the remaining seven.

```{r google-sheets-geocoder-census, fig.cap="(ref:google-sheets-geocoder-census)"}
if(knitr::is_html_output()) knitr::include_url("images/11-transform/google-sheets-geocoder-census-geographies.gif", height="330px") else knitr::include_graphics("images/google-sheets-geocoder-census.png")
```
...
* In reality only the first two columns, *unique ID* and *street address*, are required. City, state, and zip code
may be left blank if you don't have that data. But there are usually fewer exact matches for addresses with missing elements.
...
of the book, to get rid of that extra data. But if your street addresses contain apartments, you can leave them in.

US Census Geocoder has a comprehensive [overview and documentation](https://www.census.gov/programs-surveys/geography/technical-documentation/complete-technical-documentation/census-geocoder.html)
...
and displayed as a polygon (**choropleth**) map. In this case, a simple pivot table
in a spreadsheet software can solve the problem.
...
Now, imagine you are given a task to create a polygon map of total hospitals by US state,
so instead of showing individual hospitals as points, you want darker shadows of blue
to represent states with more hospitals. See Figure \@ref(fig:pivot-address) to see what
...
knitr::include_graphics("images/11-transform/pivot-address.png")
```
...
June 26, 2020. The map on the left shows total number of recorded cases per state,
and the map on the right shows cases adjusted by the state's population.
Darker colors represent higher values. Do you notice any differences in spatial patterns?

(ref:normalize-map-data) Choropleth (polygon) maps work best with normalized values.

```{r normalize-map-data, fig.cap="(ref:normalize-map-data)"}
knitr::include_graphics("images/11-transform/normalize-map-data.png")
```

Note: Both maps show Covid-19 data collected by the New York Times and published
[on GitHub](https://github.com/nytimes/covid-19-data).
In one map, we normalized (divided) values by population in each state,
...
Now, how about the map on the right? You can see that New York and its neighbors,
...
much better illustration to the story about New York being the epicenter of the Covid-19
crisis in the United States (at least in the spring of 2020).
...
Choropleth (polygon) maps work well with percentages. The good news is, humans like percentages too.
...
Raster data is a grid of cells ("pixels") of a certain size
...
knitr::include_graphics("images/11-transform/geospatial-raster-vector.png")
```

In this book, we will focus on vector data, which is based on
points, lines, and polygons. Vector data can be
...
as well as ArcGIS and QGIS all support GeoJSON.
...
In GeoJSON, coordinates are ordered in *longitude-latitude* format,
...
you know which one you are dealing with.
...
Google Earth, a free and user-friendly tool, allowed many people to
view and edit geographic data. KML files were often used
with maps powered by Google Fusion Tables, but that became history in late 2019.
...
fields.
3. The popup is interactive, and you can click and edit each property (including property names).
...
4. Click *Save* to record all marker changes to the GeoJSON. This will close the popup window,
and you will see updated markers in the JSON tab to the right of the map.
5. It may be quicker to view all data as a table instead of dealing with individual marker popups.
In **Table** tab to the right of the map, you can add, rename, and remove columns
from *all* features (markers) at once.
Table cells are also modifiable, so you can edit your data there.
6. Once you are happy with your map data, go to *Save > GeoJSON* to download the result to your computer.
...
These are useful when you have no original file to work with.
...
(which is just an instance of a polygon), and a marker (point). Let's start with the latter.
...
but different extensions), so you can easily convert a Shapefile into a web-friendly GeoJSON.
...
Consider two maps of the contiguous US states, also known as Lower-48, shown in Figure \@ref(fig:mapshaper-simplify-demo).
The map on the left is more detailed and is about 230 kilobytes, but the map on the right
...
knitr::include_graphics("images/11-transform/mapshaper-simplify-demo.png")
```
...
Mapshaper's most powerful tools are available through **Console**, which allows you to type commands for common map editing tasks.
...
In **Console**, use `-filter-fields` editing command to remove unnecessary fields.
...
Note: If you leave a space after comma, you will get a *Command expects a single value* error.
...
Now is a good time to find that .CSV file and practice your merging skills.
...
3. In **Console**, perform `-join` using a `count()` function, like that: 
```
...
As you saw in counting points in polygons example, you do not need to specify *keys* if you want to perform join
based on geographical locations between two geo files. If one of your files is a CSV, you need *keys*. 
...
You should begin by creating a *crosswalk* of towns and their health districts. It can be as simple as a two-column CSV list
of a town and its district. Because your boss didn't give you a list of towns in a spreadsheet format,
...
8. Now you are ready to merge certain towns into districts according to your uploaded CSV file. Open **Console**, and type:
```
...
Sometimes KML files are distributed in compressed KMZ format.
The easiest way to convert is to use free Google Earth Pro.
...
(ref:kmz-earth) In Google Earth Pro, right-click KMZ layer and choose *Save Place As*.
