...
3. Open [Datawrapper](https://datawrapper.de), click the *New Map* button, and select *Choropleth map* as shown in Figure \@ref(fig:datawrapper-map-choose). No login is required to create a map, but you should sign up for a free account in order to save your work and publish your map online.
...
Hint: If Datawrapper does not list your preferred map outline, you can upload your own custom geography data in GeoJSON or TopoJSON format, which you will learn more about in the [Convert to GeoJSON section of Chapter 14](convert-geojson.html).
...
```{r datawrapper-import-dataset, out.width=300, fig.cap="(ref:datawrapper-import-dataset)"}
...
```{r datawrapper-map-default, out.width=500, fig.cap="(ref:datawrapper-map-default)"}
...
12. Under the *Refine* tab, customize the legend format. For example, to convert long numbers (such as 107762) into abbreviated dollars ($108k), we selected *custom format* and inserted the code `($ 0 a)`.
...
```{r datawrapper-map-tooltip, out.width=500, fig.cap="(ref:datawrapper-map-tooltip)"}
knitr::include_graphics("images/08-map/datawrapper-map-tooltip.png")
```

14. Finally, proceed to the *Publish and Embed* screen to obtain an online link for your map, an iframe code that you will learn how to use in [Chapter 10: Embed on the Web](embed.html), and a static PNG image you can download.
...
We first introduced you to the free Tableau Public desktop application (for Mac or Windows) when building [scattercharts](scatter-tableau.html) and [filtered line charts](filtered-line-tableau.html) in Chapter 7. Now let's use the same tool to create an interactive choropleth map, and compare the process with the Datawrapper tool we learned in the prior section. Tableau Public also can create point maps for geographic names it already recognizes, but it's not ideal the tool cannot geocode new street addresses, such as "1600 Pennsylvania Avenue, Washington DC" (TODO: confirm this.) If you require custom boundaries or points (such as town names in your local language),
...
```{r tableau-data-geographic, outwidth=400, fig.cap="(ref:tableau-data-geographic)"}
