...
In the next section of this chapter, we will introduce basic steps to [copy, edit, and host a simple Leaflet map code template on GitHub](copy-leaflet.html).
...
```{r leaflet-simple-name, fig.cap="(ref:leaflet-simple-name)"}
...
(ref:leaflet-github-pages) In *Settings > GitHub Pages*, switch the source from *None* to *Master*.

```{r leaflet-github-pages, fig.cap="(ref:leaflet-github-pages)"}
knitr::include_graphics("images/08-github/leaflet-github-pages.png")
```

Note: GitHub recently announced it plans to change the default branch from *Master* to *Main* to eliminate its master-slave metaphor. **TODO:** Update repos, text, and screenshots when this happens.
...
(ref:leaflet-github-pages-link) In *Settings > GitHub Pages*, right-click your published map link to open in a new tab.

```{r leaflet-github-pages-link, fig.cap="(ref:leaflet-github-pages-link)"}
...
```{r leaflet-paste-link, fig.cap="(ref:leaflet-paste-link)"}
...
(ref:repo-code-download) Click *Code > Download Zip* to create a compressed folder of a repo on your computer.

```{r repo-code-download, fig.cap="(ref:repo-code-download)"}
...
(ref:desktop-open) In your GitHub repo on the web, click *Add file > Open with GitHub Desktop* to download and install GitHub Desktop.
...
if(knitr::is_html_output()) knitr::include_url("images/08-github/desktop-commit-push.gif") else knitr::include_graphics("images/08-github/desktop-commit-push-annotated.png")
```
