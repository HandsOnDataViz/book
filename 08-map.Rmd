...
TODO: Decide whether to mention (but not recommend)[Knight Lab's StoryMap](https://storymap.knightlab.com/) and [ESRI Story Maps](https://storymaps.arcgis.com/en/) because users cannot easily migrate data out
...
When you build a choropleth map, a great deal depends on your choice of color to represent data values, so it's important to learn key principles. Effective maps honestly display insightful geographic patterns that are clearly visible to readers. Furthermore, the best choropleth maps are readable whether they are printed in black-and-white on paper or displayed in color on a computer, and can be interpreted correctly by readers with different types of color-blindness.
...
TODO: BEGIN HERE and decide whether to begin with classes (or use default) and jump to nature of data....
...
and nine for sequential, three and eleven for diverging, and three and twelve for qualitative, which we'll discuss further below.
You can also choose palettes that are colorblind-safe and print-friendly.

ColorBrewer generates hexadecimal codes that translate into colors. For example #fffff means TODO. Also can produce RBYG and other formats TODO. You can re-type these codes into other map tools recommended in this book, or click the Export button to copy and paste this information.
...
TODO: decide whether to discuss stops first (since it appears in colorbrewer) and skip range types to the end (because it does not appear here)
