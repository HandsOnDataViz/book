...
Let's return to the world income inequality map data that we began with in the Introduction, where we created two different maps, and confessed that both were valid interpretations....
...
5. In the *Visualize* screen, next to *Select palette*, click the *wrench symbol* to open up the color settings, as shown in FIG TODO.

6. Below the light-green-to-blue color palette, experiment with the two *Types* of color ranges, as shown in FIG TODO. Click *Steps* to display values in the same range with the same color, like a staircase of steps at different heights. Now click *Continuous* to assign each value a color on the gradient, like a ramp that gradually goes up. Notice how the map menu and coloring changes according to type. For now, we'll use the default *Continuous* setting.

map-color-settings.png  (FOR ALL SETTINGS)

7. Now experiment with the *Interpolation* menu, which refers to the method of assigning values along the color range you selected above.

- *Linear* means placing the values in a straight line, from lowest to highest. This method is best when the data are evenly distributed, because it draws attention to the outliers at the light-colored low and dark-colored high ends.
- *Quantiles* means dividing the values into equal-sized groups, *Quartiles*, *quintiles*, and *deciles* means equally-sized groups of four, five, or ten, respectively. This method is best when the data are not evenly distributed, because it draws more attention by diversifying colors assigned to groups inside the range, not just the ends.
- *Rounded values* are similar to quantiles but replace decimals in the menu with nicer-looking rounded numbers.
- *Natural breaks (Jenks)* is a compromise between showing linear outliers versus equal-count quantiles.
- *Custom* allows you to choose the number of stops and place them where you wish in the color range.
...
BLEND or SIDE-BY-SIDE: map-continuous-interpolation.png and map-steps-interpolation.png


next to Go to the next screen to the *interpolation* section for options to set the map range style and cut-off points. Use the default color... Select 4 steps and the *linear* .... means.... notice that US (at 20.5%) is displayed in the 3rd out of 4 buckets, not the darkest color....

"The linear interpolation takes every value between the minimum and the maximum value and assigns it a color between the brightest and the darkest color in a linear way. Because of our uneven distribution and the outliers, our map looks like this..." and draws attention to extreme values...

https://academy.datawrapper.de/article/117-color-palette-for-your-map

map-continuous-linear.png
https://datawrapper.dwcdn.net/aetEM/

map-3steps-naturalbreaks.png
https://datawrapper.dwcdn.net/VSfdZ/

*Mislead with map range cutoffs*.

7. Make duplicate? map and title it linear scale so that we can compare it with other...

8. Go to duplicate map and select 4 steps and *quantile* scale... means.... notice that US (still at 20.5%) is now shown in the 4th bucket, the darkest color on this scale.... title this *quantile* map.

Which map is misleading? Which map is right? Both maps make valid interpretations of the data, but generate very different impressions. You may be surprised to learn that mapmakers have few universal guidelines when selecting ranges or colors.. Different mapmakers could choose to divide the legend into three, or four, or five ranges, and all would be correct. Furthermore, they could choose to divide ranges into equal intervals (such as quartiles or quintiles).... or Natural Breaks....

In general, Datawrapper Academy recommends making compromises between "honesty and usefulness" https://academy.datawrapper.de/article/117-color-palette-for-your-map
...
9. *Mislead with color choice* .... describe


10. *Mislead with map area* .... do I have a world cartogram?


- lie with map area - compare US 2016 electoral vote in standard state map (which exaggerates influence of rural states with larger geographic area) versus a cartogram (need to upload in Datawrapper?, which equalizes states of geographic area, but requires viewers to be more familiar with location)
- lying with map area is similar to map projection bias.... some projections create the appearance of larger sizes for nations in the North hemisphere than those in the South... When using map tools, understand the default projection, because few drag-and-drop tools allow you to change it.... See Leaflet?
