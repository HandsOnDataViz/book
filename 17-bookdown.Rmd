...
Inside the ref caption, do NOT use bad symbols (`<` or `>` or `"`) that will throw HTML errors into the Markdown output images. Instead, use safe symbols, such as (`*` and `-`) to designate computer instructions, such as *File - Make a Copy*.
...
- Manually replace any `<iframe...` HTML or GIF elements with their PNG substitutes, and new ALT text
- Manually delete any remaining `<iframe...` elements
