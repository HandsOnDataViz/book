...
```{r workflow-video, fig.cap="(ref:workflow-video)"}
...
- We hosted our open-access web edition on GitHub using a custom domain name (https://HandsOnDataViz.org), which we purchased and set up through [Reclaim Hosting](https://reclaimhosting.com).
...
Screenshots of two variations of the basic workflow appear in Figure \@ref(fig:workflow-desktop) and Figure \@ref(fig:workflow-atom). The first displays how to compose the book using the R Studio built-in editor, and the second shows a very similar process using the [Atom text editor](https://atom.io), which we prefer. Learn more about [GitHub Desktop and Atom text editor in Chapter 10](github-desktop-atom.html).
