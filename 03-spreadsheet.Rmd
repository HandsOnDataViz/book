...
```{r spreadsheet-terms, out.width=600, fig.cap="(ref:spreadsheet-terms)"}
 knitr::include_graphics("images/03-spreadsheet/spreadsheet-terms-annotated.png")
```
...
```{r data-formats, out.width=350, fig.cap="(ref:data-formats)"}
 knitr::include_graphics("images/03-spreadsheet/data-formats.png")
```
...
```{r mac-file-extensions, out.width=400, fig.cap="(ref:mac-file-extensions)"}
 knitr::include_graphics("images/03-spreadsheet/mac-file-extensions.png")
```

## Download to CSV or ODS Format {- #csv}
In [Chapter 2: Choose Tools to Tell Your Story](choose.html), you learned why we recommend software that supports portability, so you can migrate data to other platforms as technology evolves. Never upload important data into a tool that doesn't allow you to easily get it back out. Ideally, spreadsheet tools should allow you to export your work in generic or open-data file formats, such as Comma Separated Values (CSV) and OpenDocument Spreadsheet (ODS), to maximize your options to migrate to other platforms.
...
```{r sheets-download, out.width=600, fig.cap="(ref:sheets-download)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-download.png")
```
...
```{r file-make-copy, out.width=200, fig.cap="(ref:file-make-copy)"}
 knitr::include_graphics("images/03-spreadsheet/file-make-copy.png")
```
...
```{r my-drive-new-folder, out.width=300, fig.cap="(ref:my-drive-new-folder)"}
 knitr::include_graphics("images/03-spreadsheet/my-drive-new-folder.png")
```
...
```{r share-screen, out.width=600, fig.cap="(ref:share-screen)"}
 knitr::include_graphics("images/03-spreadsheet/share-screen.png")
```
...
```{r link-shortening, out.width=300, fig.cap="(ref:link-shortening)"}
 knitr::include_graphics("images/03-spreadsheet/link-shortening.png")
```
...
```{r drive-settings, out.width=300, fig.cap="(ref:drive-settings)"}
 knitr::include_graphics("images/03-spreadsheet/drive-settings.png")
```
...
```{r convert-uploads, out.width=750, fig.cap="(ref:convert-uploads)"}
 knitr::include_graphics("images/03-spreadsheet/convert-uploads.png")
```
...
```{r upload-file, out.width=300, fig.cap="(ref:upload-file)"}
 knitr::include_graphics("images/03-spreadsheet/upload-file.png")
```
...
```{r drive-file-icons, out.width=300, fig.cap="(ref:drive-file-icons)"}
 knitr::include_graphics("images/03-spreadsheet/drive-file-icons.png")
```
...
## Geocode Addresses with Google Sheets Add-On {- #geocode}
In this section, you'll learn how to geocode data by installing a free Google Sheets add-on tool. This allows you to geocode addresses directly inside your spreadsheet, which will be very useful when using [Leaflet map code templates in Chapter 13](leaflet.html).

Geocoding means converting addresses or location names into geographic coordinates (or x- and y-coordinates) that can be plotted on a map, as shown in Figure \@ref(fig:transform-data). For example, the Statue of Liberty in the New York City area is located at *40.69, -74.04*. The first number is the latitude and the second is the longitude. Since the equator is 0 degrees latitude, positive latitude is the northern hemisphere, and negative latitude is in the southern hemisphere. Similarly, the prime meridian is 0 degrees longitude, which passes through Greenwich, England. So positive longitude is east of the meridian, and negative longitude is west, until you reach the opposite side of the globe, roughly near the International Date Line in the Pacific Ocean.

(ref:transform-data) To map addresses, you first need to geocode them.

```{r transform-data, fig.cap="(ref:transform-data)"}
knitr::include_graphics("images/03-spreadsheet/geocode-data.png")
```

If you have just one or two addresses, you can quickly geocode them with [Google Maps](https://www.google.com/maps). Search for an address, right-click on that point, and select *What's here?* to reveal a popup window with its latitude and longitude, as shown in Figure \@ref(fig:gmaps-whats-here).

(ref:gmaps-whats-here) To geocode one address, search in Google Maps and right-click *What's here?* to show coordinates.

```{r gmaps-whats-here, fig.cap="(ref:gmaps-whats-here)"}
knitr::include_graphics("images/03-spreadsheet/gmaps-whats-here.png")
```
...
2. Go to your [Google Drive](https://drive.google.com) and create a new Google Sheet. Select the *Add-ons* menu to see the new *Geocoding by SmartMonkey* options, and select *Create Template*. The Add-on will create a new tab, named *Geocoding*, and automatically insert three sample addresses, as shown in Figure \@ref(fig:geocoding-create-template).

(ref:geocoding-create-template) In the Google Sheets *Add-On* menu, select *Geocoding by SmartMonkey -- Create Template*.

```{r geocoding-create-template, fig.cap="(ref:geocoding-create-template)"}
knitr::include_graphics("images/03-spreadsheet/geocoding-create-template.png")
```
...
(ref:geocode-details) Select *Geocode Details* to view latitude, longitude, and address found for each entry.

```{r geocode-details, fig.cap="(ref:geocode-details)"}
 knitr::include_graphics("images/03-spreadsheet/geocode-details.png")
```

When you paste your own address data into the spreadsheet to geocode it, follow these guidelines:

- Each cell requires a full address. If your original data splits street, city, state, and zip code into different columns, see how to [Combine Data into One Column in Chapter 5: Clean Up Messy Data](combine-data.html).
...
If you need a faster geocoding service for US addresses, which can handle up to 10,000 requests in one upload, see [bulk geocoding with the US Census](bulk-geocode.html) in Chapter 14: Transform Your Map Data.
...
```{r forms-new, out.width=500, fig.cap="(ref:forms-new)"}
 knitr::include_graphics("images/03-spreadsheet/forms-new.png")
```
...
```{r forms-questions, out.width=700, fig.cap="(ref:forms-questions)"}
 knitr::include_graphics("images/03-spreadsheet/forms-questions.png")
```
...
```{r forms-description, out.width=700, fig.cap="(ref:forms-description)"}
 knitr::include_graphics("images/03-spreadsheet/forms-description.png")
 ```
...
```{r forms-preview, out.width=350, fig.cap="(ref:forms-preview)"}
 knitr::include_graphics("images/03-spreadsheet/forms-preview.png")
 ```
...
```{r forms-responses, out.width=500, fig.cap="(ref:forms-responses)"}
 knitr::include_graphics("images/03-spreadsheet/forms-responses.png")
```
...
```{r sheets-select-all, out.width=400, fig.cap="(ref:sheets-select-all)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-select-all.png")
```
...
```{r sheets-sort-range, out.width=400, fig.cap="(ref:sheets-sort-range)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-sort-range.png")
```
...
```{r sheets-view-freeze, out.width=500, fig.cap="(ref:sheets-view-freeze)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-view-freeze.png")
```
...
```{r sheets-filter, out.width=300, fig.cap="(ref:sheets-filter)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-filter.png")
```
...
```{r sheets-insert-row, out.width=200, fig.cap="(ref:sheets-insert-row)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-insert-row.png")
```
...
```{r sheets-average, out.width=300, fig.cap="(ref:sheets-average)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-average.png")
```
...
if(knitr::is_html_output(excludes="markdown")) knitr::include_url("images/03-spreadsheet/sheets-drag-calc.gif", height = "185px") else knitr::include_graphics("images/03-spreadsheet/sheets-drag-calc.png")
```
...
```{r sheets-countif, out.width=250, fig.cap="(ref:sheets-countif)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-countif.png")
```
...
```{r sheets-pivot-create, out.width=450, fig.cap="(ref:sheets-pivot-create)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-pivot-create.png")
```
...
```{r sheets-pivot-rows, out.width=600, fig.cap="(ref:sheets-pivot-rows)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-pivot-rows.png")
```
...
```{r sheets-pivot-values, out.width=600, fig.cap="(ref:sheets-pivot-values)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-pivot-values.png")
```
...
```{r sheets-pivot-columns, fig.cap="(ref:sheets-pivot-columns)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-pivot-columns.png")
```
...
```{r sheets-pivot-summarize, out.width=250, fig.cap="(ref:sheets-pivot-summarize)"}
 knitr::include_graphics("images/03-spreadsheet/sheets-pivot-summarize.png")
```
...
```{r vlookup-setup, out.width=750, fig.cap="(ref:vlookup-setup)"}
 knitr::include_graphics("images/03-spreadsheet/vlookup-setup.png")
```
...
```{r vlookup-headers, out.width=500, fig.cap="(ref:vlookup-headers)"}
 knitr::include_graphics("images/03-spreadsheet/vlookup-headers.png")
```
...
```{r vlookup-solution, out.width=700, fig.cap="(ref:vlookup-solution)"}
 knitr::include_graphics("images/03-spreadsheet/vlookup-solution.png")
```
...
```{r vlookup-drag, out.width=500, fig.cap="(ref:vlookup-drag)"}
 knitr::include_graphics("images/03-spreadsheet/vlookup-drag.png")
```
...
```{r vlookup-final, out.width=500, fig.cap="(ref:vlookup-final)"}
 knitr::include_graphics("images/03-spreadsheet/vlookup-final.png")
```
...
if(knitr::is_html_output(excludes="markdown")) knitr::include_url("https://airtable.com/embed/shrOlb4XT11Xy2LP2?backgroundColor=yellow", height = "533px") else knitr::include_graphics("images/03-spreadsheet/airtable-expand.png")
```
