...
In [Chapter 5: Map Your Data](map.html), we described several drag-and-drop mapping tools designed for beginners, such as [Google My Maps](mymaps.html) and TODO: INSERT OTHERS. In this chapter, we offer more advanced map tutorials using our open-source code templates, which you can copy and modify with skills you learned in [Chapter 8: Edit and Host Code with GitHub](github.html). We built all of the templates in this chapter with [Leaflet](https://leafletjs.com), a powerful open-source code library for creating interactive maps on desktop or mobile devices.

No coding skills are required to use our first two easier-to-use templates because they pull your map data from a linked Google Sheet. The first template, [Leaflet Maps with Google Sheets](leaflet-maps-with-google-sheets), is a general-purpose tool that can display points, polygons, or polylines, using colors, styles, and linked data you enter into your Google Sheet. It also includes the option to display a table of map marker results below your map. The second template, [Leaflet Storymaps with Google Sheets](leaflet-storymaps-with-google-sheets), displays your map data and text as a scrolling narrative of chapters that guide readers through your storyline, with the option to display images, audio or video clips, and historical map backgrounds that you enter into your linked Google Sheet. If you wish to use some of these extra features, look ahead to [Chapter 11: Transform Your Map Data](transform.html) to learn how to locate addresses with our [built-in Google Sheets Geocoder](geocode.html), create and edit polygons and polylines [with the GeoJson.io tool](geojsonio.html), edit or join data with polygons [using the MapShaper tool](mapshaper.html), or georectify a scanned map to use as an overlay [with the MapWarper tool](mapwarper.html).

Our remaining Leaflet templates are designed to help users develop their map coding skills, even if you have no prior experience, but can follow instructions and are *code-curious* about how things work. TODO: DESCRIBE templates and integrate this....
...
| Leaflet Maps with CSV Data<br> ![](images/10-leaflet/leaflet-maps-with-csv.png) | Learn how to code your own Leaflet point map that pulls data from a CSV file.<br>Template with tutorial: [Leaflet Maps with CSV Data](leaflet-maps-with-csv.html)  |
| Leaflet Maps with Open Data API<br> ![](images/10-leaflet/leaflet-socrata-api-control-layers.png) | Learn how to code your own Leaflet map with an application program interface (API) to display points or polylines from open data repositories, such as Socrata and others.<br>Template with tutorial: [Leaflet Maps with Open Data API](leaflet-maps-with-api.html) |
...
#### NEW C2: Create a free Google Sheets API key to paste into the code
