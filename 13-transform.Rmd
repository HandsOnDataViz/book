...
knitr::include_graphics("images/13-transform/pivot-address-export.png")
```
...
knitr::include_graphics("images/13-transform/pivot-address-sheet.png")
```
...
## Normalize Data for Meaningful Maps {- #normalize}

TODO: Consider moving "Normalize" section out of Ch11: Transform and into a new short chapter called "Compare Appropriate Data" or "Compare Meaningful Data" or "Make Meaningful Comparisons." Expand the chapter to address numerical data in any format: text, tables, charts, or maps. Use denominators to put data into context. Explain the initial problem as one about raw versus adjusted data (see Alberto Cairo's example about highway deaths by state: raw data, or as percent of population, or as percent of miles driven). Broaden to discuss non-adjusted data versus adjusted data for various dimensions, such as population (see prior example), time (inflation, seasonal employment), geographic area (rural/suburban/urban), etc. On a related point about rates, avoid confusing "percent change" with "percentage point difference" and explain the two. The ideal location for this new chapter would be between Ch5: Clean and Ch6: Chart, because we want people to consider adjusting their data after finding and cleaning it, but before charting or mapping it.
ALSO ADD:
- avoid cherry-picking data
- beware individual versus aggregated data (people vs counties)
...
ADD
Be aware of statistically biased processes, such as a statistical sampling method that regularly yields inaccurate results. Sometimes bias arises due to a combination of human behavior and poor numerical reasoning.
...
- Later in book we'll discuss cognitive biases that may be baked into human behavior, and algorithmic biases that we sometimes build into our software...
...
knitr::include_graphics("images/13-transform/geospatial-raster-vector-annotated.png")
```

In this book, we will focus on vector data, which is based on features, which can be
points, lines, and polygons, as shown in Figure \@ref(fig:geospatial-raster-vector)b. Vector data can be
much more precise than raster data, because point coordinates
...
whether it is the surface color (as is the case with satellite imagery), temperature, or altitude.
Moreover, vector map files tend to be smaller in size than raster ones, which is important if you
...
if(knitr::is_html_output(excludes="markdown")) knitr::include_url("images/13-transform/dataviz-geojsonio-640.gif", height="466px")
```
...
knitr::include_graphics("images/13-transform/mapshaper-edit.png")
```
...
knitr::include_graphics("images/13-transform/mapshaper-clip.png")
```
...
knitr::include_graphics("images/13-transform/mapshaper-join.png")
```
...
## Georectify with MapWarper {- #mapwarper}
...
knitr::include_graphics("images/13-transform/kmz-earth.png")
```
