# Strengthen Spreadsheet Skills {#spreadsheet}
...
```{r spreadsheet-terms, fig.cap="(ref:spreadsheet-terms)"}
 knitr::include_graphics("images/02-spreadsheet/spreadsheet-terms-annotated.png")
```

In this chapter, we'll start by reviewing basic steps, such as [sharing](share.html), [uploading](upload.html), and [collecting data with online forms](forms.html). Then we'll move on to ways of organizing and analyzing your data, such as [sorting and filtering](sort.html), [calculating with formulas and functions](calculate.html), and [regrouping with pivot tables](pivot.html). Finally, we'll examine ways to connect different datasets, such as [matching columns with lookup tables](vlookup.html), and the difference between [flat spreadsheets versus relational databases](database.html). We illustrate all of these methods with beginner-level users in mind, meaning they do not require any prior background.
...
## Select your Spreadsheet Tool {- #spreadsheet-tool}
...
- `.gsheet` means Google Sheets, which also supports multi-tabbed sheets, formulas, styling, etc., but you don't normally see these on your personal computer because they are primarily designed to exist online.
...
1. Find a sample Excel file you can use on your personal computer. If you don't have one, open and save to download to your computer this [Excel file of a subset of the Hands-On Data Visualization reader public survey responses](https://handsondataviz.org/data/subset-reader-survey.xlsx)
...
```{r drive-settings, fig.cap="(ref:drive-settings)"}
...
```{r forms-description, fig.cap="(ref:forms-description)"}
...
```{r forms-preview, fig.cap="(ref:forms-preview)"}
...
```{r sheets-insert-row, fig.cap="(ref:sheets-insert-row)"}
 knitr::include_graphics("images/02-spreadsheet/sheets-insert-row.png")
```

4. Let's calculate the average level of reader experience with data visualization. Click on cell E2 in the new blank row you just created, and type an equal symbol (`=`) to start a formula. Google Sheets will automatically suggest possible formulas based on the context, and you can select one that displays the average for the range of the entire column, as shown in Figure \@ref(fig:sheets-average). If you're using a different spreadsheet tool and this formula does not automatically appear, you can type it in like this: `=AVERAGE(E3:E2894)` and press *Return* or *Enter* on your keyboard. Note that this live spreadsheet has a growing number of responses, so you will need to insert the current number of rows in the last cell reference to include all entries.
...
```{r sheets-average, fig.cap="(ref:sheets-average)"}
 knitr::include_graphics("images/02-spreadsheet/sheets-average.png")
```

Currently, the average reader experience with data visualization is around 2 on a scale from 1 (beginner) to 5 (professional), but this may change as more readers fill out the survey.

Tip: Spreadsheet tools ignore empty cells when performing calculations, so when readers leave some blank responses, it skips over those values.

5. Part of the magic of spreadsheets is that you can use the built-in click-and-drag feature to copy and paste a formula across other columns or rows, and it will automatically update its cell references. Click in cell E2, and then press and hold down on the blue dot in the bottom-right corner of that cell, which transforms your cursor into a crosshair symbol. Drag your cursor to cell F2 and let go, and show in Figure \@ref(fig:sheets-drag-calc). The formula will be automatically pasted and updated for the new column to `=AVERAGE(F3:F2894)`. Once again, since this is a live spreadsheet with a growing number of responses, your sheet will have a larger number in the last cell reference.

(ref:sheets-drag-calc) Click on the blue bottom-right dot in cell E2, then drag and drop your crosshair cursor in cell F2 to automatically paste and update the formula, as shown in these three frames.

```{r sheets-drag-calc, fig.cap="(ref:sheets-drag-calc)"}
 knitr::include_graphics("images/02-spreadsheet/sheets-drag-calc.png")
```

Warning: If you save a spreadsheet in CSV format, your calculated results will appear in the CSV sheet, but any formulas you created to produce those results will disappear. Always keep your original spreadsheet format to remind yourself how you constructed formulas.

6. Since the *Occupation* column contains a defined set of text responses, let's use a different function to count them up. Click in cell G2 and type the equal symbol (`=`) to start a new formula. Google Sheets will automatically suggest possible formulas based on the context, and you can select one that displays the count if the response is *educator* for the range of the entire column, as shown in Figure \@ref(fig:sheets-countif). If you're using a different spreadsheet tool and this formula does not automatically appear, you can type it in like this: `=COUNTIF(G3:G2894,"=educator")` and press *Return* or *Enter* on your keyboard. Once again, this live spreadsheet has a growing number of responses, so you will need to insert the current number of rows in the last cell reference to include all entries.

(ref:countif) Type `=` to start a formula and select the suggestion for count if the response is *educator*, or type it directly in with the correct range.

```{r sheets-countif, fig.cap="(ref:sheets-countif)"}
 knitr::include_graphics("images/02-spreadsheet/sheets-countif.png")
```

Currently, our readers include about 150 educators, but this will grow as more people answer the survey.  

Spreadsheet tools contain many more functions to perform numerical calculations and also to modify text, as described in this [Google Sheets Help Page](https://support.google.com/docs/topic/9054603). See more examples in the "Split data into two or more columns" and "Combine columns into one" sections in [Chapter 4: Clean Up Messy Data](clean.html).  
...
```{r sheets-pivot-rows, fig.cap="(ref:sheets-pivot-rows)"}
 knitr::include_graphics("images/02-spreadsheet/sheets-pivot-rows.png")
```

5. Next, to count up the number of responses for each entry, click the Values *Add* button and select *Occupation* again. Google Sheets will automatically summarize the values by *COUNTA*, meaning it displays the frequency of each textual response, as shown in Figure \@ref(fig:sheets-pivot-values).

(ref:sheets-pivot-values) In the *Pivot table editor*, click the Values *Add* button and select *Occupation*.

```{r sheets-pivot-values, fig.cap="(ref:sheets-pivot-values)"}
...
If you wanted to go one step further, you could also *Filter* the data to limit the pivot table results, for example, to only readers who listed 20 or more years of schooling.
...
```{r sheets-pivot-summarize, fig.cap="(ref:sheets-pivot-summarize)"}
...
Here's a common scenario where it's helpful to use the "VLOOKUP" function. Picture two data sheets with information about food banks that help feed hungry people in different parts of the US. The first sheet lists individual names of leadership staff at each food bank. The second sheet lists the address for each food bank, as shown in Figure \@ref(fig:vlookup-setup). The two sheets share a common column named "organization." Your goal is to produce one sheet that looks like a mailing list, where each row contains one individual's name, organization, and mailing address. Since we're using this small example to illustrate the concept, you could sort and manually paste in the data. But imagine that the actual list includes over 200 US food banks, so let's use "VLOOKUP" to automatically match and copy the relevant data.

(ref:vlookup-setup) Your goal is to create one mailing list that matches each name on the left sheet with the organization and address on the right sheet.

```{r vlookup-setup, fig.cap="(ref:vlookup-setup)"}
 knitr::include_graphics("images/02-spreadsheet/vlookup-setup.png")
```

1. Open this [Google Sheet of Food Bank names and addresses](https://docs.google.com/spreadsheets/d/1FYTiLHfOI2caT2lkI4i8fGbhA-oKT_MSOP1vQWWC9MA) in a new browser tab. Log into your Google Drive, and go to *File > Make a Copy* to create your own version that you can edit.

We simplified this two-sheet problem by placing both in the same Google Sheet. The first tab is called *names* and the second tab is called *addresses*. If you need to import one Google Sheet into another, go to the tab of one sheet, select *Copy to > Existing spreadsheet*, and select the name of the other sheet.
...
TODO: Consider renaming: Relational Database versus Spreadsheet
