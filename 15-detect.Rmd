...
2. Examine the data and read the notes. Overall, this data offers one way to make international comparisons about income distribution by showing "how big a slice of the pie" is held by the richest 1 percent in each nation. Each row lists a nation and its three-letter ISO code, along with the percent share of pre-tax national income held by the top 1 percent of the population, and the most recent year when this data was collected by the World Inequality Database. For example, in Brazil, the top 1 percent of the population held 28.3 percent of the nation's income in 2015, while in the United States, the top 1 percent held 20.5 percent in 2018. To be clear, there are other ways to compare the distribution of income or wealth across nations, but this is one of the better datasets available that capture this concept using one easy-to-understand variable.
...
4. Start up the [Datawrapper visualization tool](https://datawrapper.de) and select *Create a map*....Select a choropleth map... upload the CSV.... Check the data.... If you need help with these steps, review the Choropleth map with Datawrapper section in Chapter 8....

5. View your map data using the built-in histogram in screen.... Note the mean value and the shape of the distribution. Most nations are below the mean, with a handful of outliers far above the mean....

INSERT FIG of histogram screen

6. *Mislead with map ranges*. Go to the next screen to the *interpolation* section for options to set the map range style and cut-off points. Use the default color... Select 4 steps and the *linear* .... means.... notice that US (at 20.5%) is displayed in the 3rd out of 4 buckets, not the darkest color....
