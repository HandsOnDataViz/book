...
Map data is presented in layers. The top layer generally consists of a combination of *points*, *lines* (we call them *polylines*), and *polygons*. Points represent places, usually expressed as a pair of coordinates (latitude and longitude), which may be illustrated by location markers. For example, points can represent locations of restaurants, schools, or cities in zoomed-out (small-scale) maps. Polylines are connected strings of points, and we place the “poly-” prefix before “lines” to remind us that they may contain multiple branches, not just straight segments. Polylines may represent roads, rivers, or administrative boundaries. Polygons represent closed areas defined by lines, such as building footprints, parks, or country outlines.

Note: This point/polyline/polygon terminology applies to so-called vector maps that are popular on the web, and as a result relevant to this book. We briefly discuss the difference between vector and raster maps in [GeoJSON and Geospatial Data](https://handsondataviz.org/geojson.html) section of Chapter 14.
...
(ref:choropleth-use-smaller-geos) Zillow typical home values in September 2020 shown at the state level (left) and county level (right).
...
TODO ~~Above: add "New York City" and "Boston" labels to the county-level map. Decide if we should add state-level outlines and state names to county-level map. Do we wish to add a third map that's too detailed to illustrate the Goldilocks effect? -- zillow only goes down to zipcode level with a lot of missing data so won't be effective ~~
...
- *Natural breaks (Jenks)* groups close values together while maximizing the "distance" between different groups. It  offers a compromise between linear (which emphasizes the extreme ends) and quantiles (which emphasize internal diversity).
...
If you want to embed the map as an iframe, from the main kebab menu to the right of the map title, choose *Embed on my site* (Figure \@ref(fig:mymaps-embed)). This will generate an HTML code snippet that you can copy to your website.
...
In the top menu, click *Map* (globe icon between a scatter chart icon and a calendar). You will see an updated layout of the studio, with the map in the middle, and *Map Layers* and *Map Settings* items in the side menu on the left, as shown in Figure \@ref(fig:socrata-configigure).
