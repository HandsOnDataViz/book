...
(ref:sheets-drag-calc) Click on the blue bottom-right dot in cell E2, then hold-and-drag your crosshair cursor in cell F2, and let go to automatically paste and update the formula, as shown in these three frames.
...
(ref:countif) Select or enter a formula that counts responses if the entry is *educator*.
...
Here's a common scenario where it's helpful to use the "VLOOKUP" function. Picture two data sheets with sample data about food banks that help feed hungry people in different parts of the US, drawn from [Feeding America: Find Your Local Food Bank](https://www.feedingamerica.org/find-your-local-foodbank). The first sheet lists individual names of leadership staff at each food bank. The second sheet lists the address for each food bank, as shown in Figure \@ref(fig:vlookup-setup). The two sheets share a common column named "organization." Your goal is to produce one sheet that looks like a mailing list, where each row contains one individual's name, organization, and mailing address. Since we're using this small sample to illustrate the concept, you could sort and manually paste in the data. But imagine that the actual sheets includes over 200 US food banks and many more individuals. So let's use "VLOOKUP" to automatically match and paste data for us on this small sample, to prepare for much larger datasets in the future.
...
(ref:vlookup-solution) The VLOOKUP formula in *names* sheet cell C2 seeks to match cell B2 across columns A to E in the *addresses* sheet.

```{r vlookup-solution, fig.cap="(ref:vlookup-solution)"}
 knitr::include_graphics("images/02-spreadsheet/vlookup-solution-annotated.png")
```

Let's break down each part of the formula you entered in the *names* sheet, cell C2:

- `B2` = The search_key, meaning the cell you wish to match in the 1st sheet (the organization in the *names* sheet)
- `'addresses'!A:E` = The range, meaning you are searching for your match and results across columns A to E in the *addresses* sheet.
- `2` = The index, meaning your desired result appears in the 2nd column of the range above (columns A to E in the *addresses* sheet).
- `false` = Find exact matches only.

5. After you enter the full VLOOKUP formula, it should display the exact match for the first organization, the Central Texas Food Bank, whose address is 6500 Metropolis Dr. Click and hold down on the blue dot in the bottom-right corner of cell C2, and drag your crosshair cursor across columns D to F and let go, which will automatically paste and update the formula for the city, state, and zip columns, as shown in Figure \@ref(fig:vlookup-drag).
...
```{r vlookup-drag, fig.cap="(ref:vlookup-drag)"}
...
```{r vlookup-final, fig.cap="(ref:vlookup-final)"}
 knitr::include_graphics("images/02-spreadsheet/vlookup-final.png")
```

Warning: If you save this spreadsheet in CSV format, your calculated results will appear in the CSV sheet, but any formulas you created to produce those results will disappear. Always keep your original spreadsheet format to remind yourself how you constructed formulas.

You've successfully created a mailing list---including each person's name, organization, and full address---using the VLOOKUP function to match and paste data from two sheets. Now that you understand how to use formulas to connect different spreadsheets, the next section introduces you to the differences between flat spreadsheets versus relational databases, and why you might want to use one over the other.
