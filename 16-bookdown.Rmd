...
At the end of each chapter, insert **Summary** using bold type, rather than a hashtag header, to avoid creating several unnecessary web pages named "Summary" in the HTML edition. Although this does not match the O'Reilly style guide, it can easily be fixed in book production. 
...
```{r screenshot-tall, fig.pos='h', out.width=200, fig.cap="(ref:screenshot-tall)"}
 knitr::include_graphics("images/16-bookdown/screenshot-tall.png")
```
...
```{r sample-map, fig.pos='h', fig.cap="(ref:sample-map)"}
