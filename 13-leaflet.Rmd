...
4. Feel free to remove any other content on the README page that you do not wish to keep.
...
- Marker Color: Insert any standard color name such as `blue` or `darkblue`, or insert a web color code such as `#775307` or `rgba(200,100,0,0.5)`. See options at [W3Schools Color Names](https://www.w3schools.com/colors/colors_names.asp).
...
- Image: Insert link to an external image link, or upload a small image to the *media* folder in your GitHub repo and add its pathname, such as `media/trinity-college.jpg`.
...
TODO above: update the code demo to include a sample image in the media file, and edit text above to match
...
Optional custom markers: To create your own custom marker, such as a thumbnail photo icon, use any image editing tool to reduce a photo to a square of 64 x 64 pixels. Save it in PNG format and choose a filename using all lower-case characters with no spaces. Upload the image to the *media* folder in your GitHub repo. In the Marker Icon column, set the pathname in this format: `media/custom-trinity-64.png`. In the Custom Size column, set the dimensions to `64x64` or similar, such as `40x40` if desired.

TODO above: update the code demo to include a thumbnail photo icon in the media file, and edit text above to match.
...
- In the *Polylines* tab, enter the GeoJSON URL pathname to the file you uploaded to your GitHub repo, such as `geodata/polylines-bike-lanes.geojson`. Then insert a Display Name, Description, and Color.

To display polygons:

- In the *Polygons* tab, make sure *Polygon Legend Position* (cell B4) is visible by selecting *topleft* or a similar position.
- Also, in *Polygon GeoJSON URL* (cell B6) enter the pathname to the file you uploaded to your GitHub repo, such as `geodata/polygons-town-population.geojson`.
...
TODO: Create and insert a new version of the template demo
...
- E) Geocode locations and customize new markers in the Points tab.  RENAME
- F) RENAME*****
...
4) In your new copy of the code repo, click on Settings, scroll down to the GitHub Pages area, select Main, and Save. This publishes your code to a live map on a public website that you control.
