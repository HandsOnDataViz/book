...
Sometimes you need to create a map that cannot be made easily with drag-and-drop tools, because you need to customize its appearance or show some combination of point, polygon, or polyline data. One solution is to make a copy of our Leaflet Maps with Google Sheets template, as shown in Figure \@ref(fig:lmwgs-map). This introductory-level template allows you to choose custom point icons and choropleth color palettes, and to display different combinations of map data layers. It's a simpler code template for newer users because you enter your map data and modify settings in a linked Google Sheet, as shown in Figure \@ref(fig:lmwgs-sheet), and upload images or geographic data into a template code folder in your GitHub repository.
...
(ref:lmwgs-sheet) View the [online Google Sheet template](https://docs.google.com/spreadsheets/d/1ZxvU8eGyuN9M8GxTU9acKVJv70iC3px_m3EVFsOHN9g) that feeds data into the Leaflet map above.
...
TODO: Add intro text; mention (but not recommend)[Knight Lab's StoryMap](https://storymap.knightlab.com/) and [ESRI Story Maps](https://storymaps.arcgis.com/en/) because users cannot easily migrate data out
