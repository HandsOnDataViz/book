...
Explore our Leaflet map templates and you'll also see how they refer to different code components, such as basemap tiles from [various open-access online providers](https://leaflet-extras.github.io/leaflet-providers/preview/), such as Carto, Esri, Stamen, and Open Street Map, that allow you to zoom into background maps. You'll also see data files to place information about points, polygons, or polylines on top of the map, usually in CSV or [GeoJSON format---see chapter 11](convert-geojson.html), with names similar to `data.csv` or `map.geojson`. If you're new to coding, creating Leaflet maps can be a great place to start and quickly see the results of what you've learned. To help you solve problems that may arise, see [Fix Common Mistakes](fix.html) in the appendix.

Table: (\#tab:map-templates) Map Templates and Tutorials

| Map Templates | Best use and tutorials in this book |
| --- | --- |
| Leaflet Maps with Google Sheets<br> ![](images/10-leaflet/leaflet-maps-with-google-sheets.png) | Best to show interactive points, polygons, or polylines, using your choice of colors, styles, and icons, based on data loaded into your linked Google Sheet (or CSV file) and GitHub repository. Includes option to display a table of point map markers next to your map.  <br>Template with tutorial: [Leaflet Maps with Google Sheets](leaflet-maps-with-google-sheets.html) |
| Leaflet Storymaps with Google Sheets<br> ![](images/10-leaflet/leaflet-storymaps-with-google-sheets.png) | Best to display your map as a scrolling narrative of chapters to guide readers through a storyline, with the option to include paragraphs of text, images, audio or video clips, and historical map backgrounds loaded into your linked Google Sheet (or CSV) and GitHub repo. <br>Template with tutorial: [Leaflet Storymaps with Google Sheets](leaflet-storymaps-with-google-sheets.html) |
| Leaflet Maps with CSV Data<br> ![](images/10-leaflet/leaflet-maps-with-csv.png) | Learn how to code your own Leaflet point map that pulls data from a CSV file in your GitHub repo.<br>Template with tutorial: [Leaflet Maps with CSV Data](leaflet-maps-with-csv.html)  |
| Leaflet Maps with Open Data API<br> ![](images/10-leaflet/leaflet-socrata-api-control-layers.png) | Learn how to code your own Leaflet map with an application program interface (API) that pulls content directly from open data repositories, such as Socrata and others.<br>Template with tutorial: [Leaflet Maps with Open Data API](leaflet-maps-with-api.html) |
...
Answer: Copy and customize our open-source template for Leaflet Maps with Google Sheets. Control the map options display data that you upload to your Google Sheet and GitHub repository. No coding skills required, other than pasting one line of code to link your map with your sheet.
...
### Before You Begin {-}
- You must have a [Google Drive account](http://drive.google.com), which is freely available, and know how to [File > Make a Copy in Google Sheets](copy.html) as described in Chapter 2.
- You must have a [GitHub account](http://github.com), which is freely available, and know how to [Edit and Host Code with GitHub](github.html) as described in Chapter 8.
- If you invite people to visit your copy of this template, you must [Get Your Own Free Google Sheets API Key](google-sheets-api-key.html) to insert into the code, so that our key is not overused. This step requires a personal Google account, *not* a Google Suite account provided by your school or business. *TODO:* confirm this detail

### Tutorial Outline {-}
Since this tutorial involves many steps, here's an outline of the major ones to help guide you through the details below. In the first half, you will create and publish your copies of our GitHub and Google Sheets templates:
...
- G) Upload and display your polygon GeoJSON data
- H) Upload and display your polyline GeoJSON data
...
- Reminder: If you invite people to visit your map, [Get Your Own Free Google Sheets API Key](google-sheets-api-key.html) to insert into the code, to avoid overusing our key.
...
knitr::include_graphics("images/10-leaflet/leaflet-github-pages2.png")
```
...
5. At the top of your browser, copy your Google Sheet browser address (which usually ends in `...XYZ/edit#gid=0`). Do *NOT* copy the *Published to the web* address (which usually ends in `...XYZ/pubhtml`), as shown in Figure \@ref(fig:sheets-publish).
...
```{r sheets-publish, out.width=500, fig.cap="(ref:sheets-publish)"}
 knitr::include_graphics("images/10-leaflet/sheets-publish-annotated.png")
```
...
2. Paste *your* Google Sheet browser address (which usually ends in `...XYZ/edit#gid=0`) to replace the existing address, as shown in Figure TODO: INSERT AFTER CODE UPDATE. Do *NOT* erase the single quotation marks or the semicolon at the end.
...
Open the browser tab that displays your live map, which follows this format: `https://USERNAME.github.io/leaflet-maps-with-google-sheets`.

Give your browser a "hard refresh" to [bypass any saved content in your cache](https://en.wikipedia.org/wiki/Wikipedia:Bypass_your_cache) and re-download the entire web page from the server, using one of these key combinations:
...
If your live map displays your new map title or any other info you entered into your Google Sheets *Options* tab, then congratulations! Or if your map content does not update after 30 seconds and a second "hard refresh," then see the [Fix Common Problems](fix.html) section of the appendix.
...
## Create Your Own Google Sheets API Key {- #google-sheets-api-key}
TODO: Explain why you need to create a free Google Sheets API Key to allow the code template to read data from your Google Sheet. (This is a new requirement with version X of our template, beginning in August 2020, in order to work with Google Sheets API version 4.)

Reminders:

- you need a personal Google account, not a Google Suite account issued by your school or business
- if you already did this step for another Leaflet Map, you can copy and use your existing Google Sheets API key


- Visit https://console.developers.google.com/ and log in to your Google account. The Google Cloud Platform may ask you to identify your country and agree to terms of service.

- Click on *Create a Project* on the opening screen. Or if you don't see that option, go to the upper-left drop-down menu to *Select a project > New project*. IMAGE google-api-create.png

- Give your new project a meaningful short name to remind you of its purpose, such as `handsondataviz`. You do not need to create an organization or parent folder. Click Create. IMAGE google-api-name.png

- Enable Google Sheets API inside your Google cloud project. To do so,
    - From the top menu, press `+ Enable APIs and Services`   IMG google-api-enable.png
    - Use the search bar to find Google Sheets API and select it.   IMG google-api-search.png
    - In the next screen click Click Enable.   IMG google-api-select.png

  - In the left sidebar menu, click *Credentials*, then click *+ Create Credentials* and select *API key*. IMG google-api-credentials.png

- A window with the generated API key will appear. Copy it. IMG google-api-key.png

- In that window, click Restrict key. In the new window, under API restrictions, choose Restrict key radio button. In the dropdown that appears, choose Google Sheets API. Click Save. IMG google-api-restrict.png

- COPY see text below -- IMG google-api-copy.png

Copy your Google API key to `google-doc-url.js` file inside a GoogleApiKey variable. Once you save it go GitHub you may get a notification from GitHub’s GitGuardian bot saying you have an exposed API key. Don’t worry about it – this key can only be used with Google Sheets, and it’s free. You did not attach billing information to it, so there is no way you can get charged.
